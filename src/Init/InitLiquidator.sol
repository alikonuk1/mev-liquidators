...
contract InitLiquidator is Ownable {
    ...
    address private constant INIT_CORE =
        0x972BcB0284cca0152527c4f70f8F689852bCAFc5; // Mantle
    ...
    function calculateRepayShare(uint256 posId) public returns (uint256) {
        ...
        (address[] memory debtPools, uint256[] memory debtShares) = IPosManager(
            POS_MANAGER
        ).getPosBorrInfo(posId);
        ...
        uint256 healthFactor = IInitCore(INIT_CORE).getPosHealthCurrent_e18(
            posId
        );
        ...
        uint256 maxHealthAfterLiquidation_e18 = IConfig(config)
            .getMaxHealthAfterLiq_e18(mode);

        uint256 scaledHealth = maxHealthAfterLiquidation_e18 - healthFactor;
        require(scaledHealth > 0, "Skip zero scaled health.");
        uint256 scaledSharesToRepay = (debtShare * scaledHealth) /
            305000000000000000;
    ...
    function calculateLiquidationPrice(
        ...
        uint256 totalBorrowCredit = IInitCore(INIT_CORE)
            .getBorrowCreditCurrent_e36(posId);
        uint256 totalCollateralCredit = IInitCore(INIT_CORE)
            .getCollateralCreditCurrent_e36(posId);

        (
            address[] memory collPools,
            uint256[] memory collAmts,
            ,
            ,
        ...
        uint256 debtAmt = ILendingPool(debtPools[0]).debtShareToAmtCurrent(
            debtShares[0]
        );
        require(debtAmt > 0, "debtAmt is zero");
        uint16 mode = IPosManager(POS_MANAGER).getPosMode(posId);
        TokenFactors memory collFactor = IConfig(IInitCore(INIT_CORE).config())
            .getTokenFactors(mode, collPools[0]);
        TokenFactors memory borrFactor = IConfig(IInitCore(INIT_CORE).config())
            .getTokenFactors(mode, debtPools[0]);
        ...
        if (isCollateral) {
            ...
            uint256 liquidationPrice = totalBorrowCredit.mulDiv(
                ONE_E18,
                denominator
            );
            ...
            uint256 liquidationPrice = totalCollateralCredit.mulDiv(
                ONE_E18,
                denominator
            );
            return liquidationPrice;
