...
interface IPosManager {
    event SetMaxCollCount(uint256 maxCollCount);
    ...
    struct PosCollInfo {
        ...
        mapping(address => uint256) collAmts; // collateral token to collateral amts mapping
    ...
    struct PosBorrInfo {
        ...
        mapping(address => uint256) debtShares; // debt token to debt shares mapping
    ...
    function nextNonces(address _owner) external view returns (uint256 nextNonce);
    ...
    function pendingRewards(uint256 _posId, address _rewardToken)
        external
        view
        returns (uint256 amt);
    ...
    function isCollateralized(address _wLp, uint256 _tokenId)
        external
        view
        returns (bool);
    ...
    function getPosBorrInfo(uint256 _posId)
        external
        view
    ...
    function getPosBorrExtraInfo(uint256 _posId, address _pool)
        external
        view
        returns (uint256 totalInterest, uint256 lastDebtAmt);
    ...
    function getPosCollInfo(uint256 _posId)
        external
        view
        returns (
    ...
    function getCollAmt(uint256 _posId, address _pool)
        external
        view
        returns (uint256 amt);
    ...
    /// @return amt collateral amount
    function getCollWLpAmt(uint256 _posId, address _wLp, uint256 _tokenId)
        external
        view
        returns (uint256 amt);
    ...
    function getPosInfo(uint256 _posId)
        external
        view
        returns (address viewerAddress, uint16 mode);

    /// @dev get position mode
    /// @param _posId position id
    /// @return mode position's mode
    function getPosMode(uint256 _posId) external view returns (uint16 mode);

    /// @dev get pool's debt shares for the position
    /// @param _posId position id
    /// @param _pool lending pool address
    /// @return debtShares debt shares
    function getPosDebtShares(uint256 _posId, address _pool)
        external
        view
    ...
    /// @param _index index
    /// @return posId pos id
    function getViewerPosIdsAt(address _viewer, uint256 _index)
        external
        view
    ...
    function getViewerPosIdsLength(address _viewer)
        external
        view
    ...
    function updatePosDebtShares(uint256 _posId, address _pool, int256 _debtShares)
        external;
    ...
    function updatePosMode(uint256 _posId, uint16 _mode) external;
    ...
    function addCollateral(uint256 _posId, address _pool)
        external
    ...
    function addCollateralWLp(uint256 _posId, address _wLp, uint256 _tokenId)
        external
        returns (uint256 amtIn);
    ...
    function removeCollateralWLpTo(
        uint256 _posId,
        address _wLp,
        uint256 _tokenId,
        uint256 _amt,
    ...
    function createPos(address _owner, uint16 _mode, address _viewer)
        external
        returns (uint256 posId);
    ...
    function harvestTo(uint256 _posId, address _wlp, uint256 _tokenId, address _to)
        external
    ...
    function claimPendingRewards(uint256 _posId, address[] calldata _tokens, address _to)
        external
    ...
    function isAuthorized(address _account, uint256 _posId)
        external
        view
        returns (bool);
    ...
    function setPosViewer(uint256 _posId, address _viewer) external;
}
