...
interface IConfig {
    ...
    function isAllowedForBorrow(uint16 _mode, address _pool)
        external
        view
        returns (bool);
    ...
    function isAllowedForCollateral(uint16 _mode, address _pool)
        external
        view
        returns (bool);
    ...
    function getTokenFactors(uint16 _mode, address _pool)
        external
        view
        returns (TokenFactors memory tokenFactors);
    ...
    function getMaxHealthAfterLiq_e18(uint16 _mode)
        external
        view
        returns (uint256 maxHealthAfterLiq_e18);

    /// @dev get the current mode status
    /// @param _mode mode id
    /// @return modeStatus mode status (collateralize, decollateralize, borrow or repay)
    function getModeStatus(uint16 _mode)
        external
        view
        returns (ModeStatus memory modeStatus);
    ...
    function setMaxHealthAfterLiq_e18(uint16 _mode, uint64 _maxHealthAfterLiq_e18)
        external;
