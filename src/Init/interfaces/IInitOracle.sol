...
interface IInitOracle is IBaseOracle {
    ...
    event SetMaxPriceDeviation_e18(address indexed token, uint256 maxPriceDeviation_e18);

    /// @dev return the oracle token's primary source
    /// @param _token token address
    /// @return primarySource primary oracle address
    function primarySources(address _token)
        external
        view
        returns (address primarySource);

    /// @dev return the oracle token's secondary source
    /// @param _token token address
    /// @return secondarySource secoundary oracle address
    function secondarySources(address _token)
        external
        view
        returns (address secondarySource);
    ...
    function maxPriceDeviations_e18(address _token)
        external
        view
        returns (uint256 maxPriceDeviation_e18);
    ...
    function getPrices_e36(address[] calldata _tokens)
        external
        view
        returns (uint256[] memory prices_e36);

    /// @dev set primary source for tokens
    /// @param _tokens token address list
    /// @param _sources the primary source address for each tokens
    function setPrimarySources(address[] calldata _tokens, address[] calldata _sources)
        external;

    /// @dev set secondary source for tokens
    /// @param _tokens token address list
    /// @param _sources the secondary source address for each tokens
    function setSecondarySources(address[] calldata _tokens, address[] calldata _sources)
        external;
    ...
    function setMaxPriceDeviations_e18(
        address[] calldata _tokens,
        uint256[] calldata _maxPriceDeviations_e18
    ) external;
}
